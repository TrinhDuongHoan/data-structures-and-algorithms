...
void push_back(DaThuc &daThuc, float heSo, int soMu)
{
    ...
    if (daThuc.pHead == NULL)
        daThuc.pHead = daThuc.pTail = node;
    else
    {
...
DaThuc plusDaThuc(DaThuc a, DaThuc b)
{
    ...
    while (p != NULL && q != NULL)
    {
        if (p->data.soMu == q->data.soMu)
        {
            ...
            if (heSo)
                push_back(daThuc, heSo, p->data.soMu);
    ...
    while (p != NULL)
    {
        ...
        else
        {
            ...
            else if (abs(heSo) == 1)
            {
                ...
                printf("%s %.0fx^%d ", (heSo > 0 ? "+" : "-"), abs(heSo), soMu);
        }
        p = p->pNext;
    }
    printf("\n");
...
int main()
{
    ...
    // Phép cộng
    printf("A + B = ");
    display(plusDaThuc(a, b));

    // Phép trừ
    printf("A - B = ");
    display(minusDaThuc(a, b));

    // Phép nhân
    printf("A * B = ");
    display(multiDaThuc(a, b));

    // Phép chia
    printf("A / B = ");
    display(divideDaThuc(a, b, c));
    printf("A / B Du C = ");
    display(c);
    return 0;
}